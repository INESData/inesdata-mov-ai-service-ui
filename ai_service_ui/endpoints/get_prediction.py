"""Bus arrival prediction logic."""
import json

import requests
import streamlit as st
from ai_service_ui.endpoints import transfer
from ai_service_ui.settings import settings
from ai_service_ui.utils import authenticate
...
def get_prediction(stop_id:str, token:str):
    """Obtain time arrival prediction.

    Args:
        stop_id (str): Stop number
        token (str): Token
    """
    if stop_id != "-":
        st.session_state["stop_id"] = stop_id
        download_url = settings.prediction_api_url.format(stop_id=stop_id, line_id=st.session_state["line_id"])
...
def bus_arrival_predictor():
    """Returns the bus arrival prediction after authentication."""
